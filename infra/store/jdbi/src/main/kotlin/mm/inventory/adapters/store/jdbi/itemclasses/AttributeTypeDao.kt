...
interface AttributeTypeDao {
    ...
    @SqlQuery("SELECT a.name, a.scalar, u.code AS unit_code, u.name AS unit_name, a.scalar, v.code, v.value FROM Attribute_types a LEFT OUTER JOIN Attribute_type_values v ON a.name = v.attribute_type_name LEFT OUTER JOIN Units u ON a.unit = u.code WHERE a.name=:name ORDER BY a.name, v.code")
