...
internal interface ItemClassDao {
    ...
    @SqlQuery("""
        SELECT name, version, description, unit 
        FROM Item_classes 
    ...
    @SqlQuery("""
        SELECT a.attribute_type, at.name, at.scalar, u.code as unit_code, u.name as unit_name 
        FROM Attributes a 
        JOIN Attribute_Types at ON a.attribute_type = at.name 
        LEFT OUTER JOIN Units u ON at.unit = u.code 
        WHERE item_class_name=? AND item_class_version=?""")
    fun selectAttributesWithTypesForItemClass(itemClassName: String, itemClassVersion: Long): List<AttributeWithTypeRec>

    @SqlQuery("""
        SELECT attribute_type_name, code, value 
        FROM Attribute_Type_Values 
        WHERE attribute_type_name IN (
            SELECT attribute_type 
            FROM Attributes 
            WHERE item_class_name=? 
            AND item_class_version=?)""")
    ...
    @SqlUpdate("""
        UPDATE Item_classes_version_counters 
        SET last_version=(
            SELECT MAX(version) 
            FROM Item_classes 
            WHERE name=:itemClassName) 
        WHERE name=:itemClassName""")
    fun revertCounter(itemClassName: String): Int

    @SqlUpdate("""
        INSERT INTO Item_classes(name, version, complete, description, unit) 
        VALUES (:itemClass.name, :itemClass.version, FALSE, :itemClass.description, :itemClass.unit)""")
    fun insertItemClass(itemClass: ItemClassRec): Int

    @SqlUpdate("""
        INSERT INTO Attributes(item_class_name, item_class_version, attribute_type) 
        VALUES (:attribute.itemClassName, :attribute.itemClassVersion, :attribute.attributeType)""")
    fun insertAttribute(attribute: AttributeRec): Int

    @SqlUpdate("""
        DELETE FROM Attributes 
        WHERE item_class_name=:attribute.itemClassName 
            AND item_class_version=:attribute.itemClassVersion 
            AND attribute_type=:attribute.attributeType""")
    fun deleteAttribute(attribute: AttributeRec): Int

    @SqlUpdate("""
        UPDATE Item_classes 
        SET complete=TRUE 
        WHERE name=:id.id AND version=:id.version AND complete=FALSE""")
